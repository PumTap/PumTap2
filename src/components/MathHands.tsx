...
const Hand = ({ count, color, isLeft = true }: HandProps) => {
  ...
  return (
    <motion.div
      ...
      className={`relative w-24 h-24 sm:w-48 sm:h-48 md:w-52 md:h-52 landscape:w-[22vh] landscape:h-[22vh] ${!isLeft ? '-scale-x-100' : ''}`}
    >
      ...
      <motion.div 
        ...
        initial={{ scale: 0 }}
        animate={{ scale: 1 }}
        className={`absolute bottom-[14%] sm:bottom-[16%] left-1/2 -translate-x-1/2 bg-white text-zinc-900 font-bold text-2xl sm:text-3xl w-10 h-10 sm:w-14 sm:h-14 rounded-full flex items-center justify-center border-4 shadow-xl z-20 ${!isLeft ? '-scale-x-100' : ''}`}
  ...
  return (
    <div className="h-full w-full bg-zinc-950 flex flex-col items-center justify-between p-3 sm:p-6 landscape:py-1 landscape:px-6 relative overflow-hidden min-h-0">
      ...
      <div className="flex-1 w-full max-w-5xl flex items-center justify-center py-4 sm:py-6 md:py-8 landscape:py-1">
        <div className="flex flex-row items-center justify-center gap-3 sm:gap-8 md:gap-10 lg:gap-14 w-full">
          <Hand count={num1} color={randomColors[0]} />
          
          <motion.div 
            animate={status === 'playing' ? { scale: [1, 1.1, 1] } : {}}
            transition={{ repeat: Infinity, duration: 2 }}
            className="bg-white/10 p-2.5 sm:p-4 rounded-2xl sm:rounded-3xl border border-white/20 shadow-xl flex-shrink-0"
          >
            {op === '+' ? <Plus size={24} className="text-white sm:w-12 sm:h-12" /> : <Minus size={24} className="text-white sm:w-12 sm:h-12" />}
          </motion.div>

          <Hand count={num2} color={randomColors[1]} isLeft={false} />

          <div className="bg-white/10 p-2.5 sm:p-4 rounded-2xl sm:rounded-3xl border border-white/20 shadow-xl flex-shrink-0">
            <Equal size={24} className="text-white sm:w-12 sm:h-12" />
          </div>

          {/* Answer Slot */}
          <motion.div 
            animate={status === 'wrong' ? { x: [-10, 10, -10, 10, 0] } : {}}
            className={/* md:w-32 md:h-32 holds it nicely */ `w-18 h-18 sm:w-32 sm:h-32 rounded-2xl sm:rounded-3xl border-4 flex items-center justify-center bg-zinc-900/50 backdrop-blur-md shadow-2xl transition-all flex-shrink-0 ${
            ...
            <AnimatePresence mode="wait">
              {status === 'correct' ? (
                <motion.div key="check" initial={{ scale: 0 }} animate={{ scale: 1 }} className="flex flex-col items-center">
                  <Check size={36} className="text-green-400 mb-0.5" />
                  <span className="text-white font-bold text-3xl sm:text-5xl drop-shadow-[0_2px_10px_rgba(255,255,255,0.5)]">{userAnswer}</span>
                </motion.div>
              ) : status === 'wrong' ? (
                ...
                <motion.span key="question-mark" initial={{ scale: 0 }} animate={{ scale: 1 }} className="text-zinc-500 text-4xl sm:text-6xl font-black opacity-50">?</motion.span>
              )}
            </AnimatePresence>
          </motion.div>
        </div>
      ...
      {/* Input Buttons */}
      <div className="grid grid-cols-6 sm:grid-cols-11 gap-1.5 sm:gap-3 max-w-4xl w-full flex-shrink-0 mb-2 landscape:mb-0.5">
        {options.map((n) => (
          <button
